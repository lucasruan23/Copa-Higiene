...
// ===== PERGUNTAS =====
const ALL_Q = [
  {cat:"⚽ Aquecimento",q:"A OMS recomenda que as mãos sejam higienizadas por pelo menos quanto tempo com água e sabão?",opts:["5 segundos","20 segundos","1 minuto","5 minutos"],ans:1,exp:"A OMS recomenda pelo menos 20 segundos."},
  ...
  {cat:"🌍 Mundial",q:"A lavagem correta das mãos pode reduzir em quanto os casos de diarreia em crianças?",opts:["Cerca de 10%","Cerca de 25%","Cerca de 48%","Apenas 5%"],ans:2,exp:"Estudos indicam redução de até 48%."},
  ...
  {cat:"🏆 Regulação",q:"No Brasil, qual órgão publica normas sobre higienização das mãos em serviços de saúde?",opts:["SUS","ANVISA","Ministério do Esporte","INMETRO"],ans:1,exp:"A ANVISA é o órgão regulador responsável."},
  {cat:"🧪 Ciência",q:"Qual patógeno hospitalar tem transmissão fortemente reduzida com higiene das mãos?",opts:["MRSA","Vírus da gripe sazonal","Toxoplasma gondii","Plasmodium falciparum"],ans:0,exp:"O MRSA é frequentemente transmitido pelas mãos dos profissionais."},
  ...
  {cat:"🎯 Pênalti Final",q:"Qual é a forma correta de secar as mãos após a lavagem?",opts:["Papel toalha descartável, secando completamente","Sacudir as mãos no ar","Secar na roupa","Deixar secar ao natural"],ans:0,exp:"Mãos úmidas transmitem muito mais microrganismos."},
  ...
  {cat:"👋 Unhas",q:"Qual característica de unhas é recomendada para quem presta assistência direta?",opts:["Unhas curtas, sem esmalte ou alongamento","Unhas grandes com esmalte claro","Unhas médias com base incolor","Unhas postiças bem lixadas"],ans:0,exp:"Unhas curtas e sem esmalte facilitam a higienização."},
  ...
  {cat:"👨‍👩‍👧 Público Geral",q:"Em escolas e eventos esportivos, incentivar a higiene das mãos impacta principalmente:",opts:["Notas escolares","Transmissão de infecções","Conforto térmico","Desempenho físico"],ans:1,exp:"Menos infecções significam menos faltas e menor transmissão."}
];
...
function registrarTime(salaId, time) {
  const sala = getSala(salaId);
  if (!sala.times[time]) {
    sala.times[time] = { gols: 0, respondeu: false };
    if (!sala.timesEsperados.includes(time)) sala.timesEsperados.push(time);
  }
}

function todosResponderam(sala) {
  return sala.timesEsperados.every(t => sala.times[t].respondeu);
}
...
app.post('/responder', (req, res) => {
  ...
  res.json({
    ...
    proximaPerguntaIndex: s.perguntaIndex,
    fimDeJogo: s.perguntaIndex >= s.perguntas.length,
  });
});
...
app.post('/resetar', (req, res) => {
  const { sala = '1' } = req.body;
  if (salas[sala]) {
    const timesEsperados = salas[sala].timesEsperados;
    criarSala(sala);
    timesEsperados.forEach(t => registrarTime(sala, t));
  }
  res.json({ ok: true });
});
